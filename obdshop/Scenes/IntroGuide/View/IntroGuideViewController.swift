...
class IntroGuideViewController: BaseViewController, StoryboardLoadable {
    ...
    override func viewDidLoad() {
        ...
        let siteURL = "https://woo.zhengblog.com/wp-json/wc/v2/products/categories"
      ...
      /*
        ...
        Alamofire.request(url,
            ...
            .responseData { response in
                if response.result.value != nil{
                    print(response.result.value)
                    let decoder = JSONDecoder()
                    let products = try! decoder.decode([ProductCategory].self, from:  response.result.value!   )
                    let test = "ttttt"
                    print(test)
                }else{
        ...
        Alamofire.request(url,
                          ...
                          headers:headers)
            .validate()
            .responseDecodableObject(keyPath: nil, decoder: decoder) { (response: DataResponse<[ProductCategory]>) in
                let productcat = response.result.value
                print(productcat)
        }
